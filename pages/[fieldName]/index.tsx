...
import { circlesGql, circlesLinkGql } from "../../graphql/circles";
...
export const getStaticPaths: GetStaticPaths = async () => {
    let paths: { params: { fieldName: string } }[] = [];

    try {
        let res = await graphQLClient.request(circlesLinkGql);
        paths = res.circles.map((circle: { name: string }) => ({
            params: { fieldName: circle.name },
        }));
    } catch (err) {
